    ...
    >
      ...
      >
        ...
        <div className="border-b border-up-orange/15 px-5 py-4">
          <div className="text-[10px] uppercase tracking-wider text-up-dim">nova votação</div>
          <input
            autoFocus
            value={title}
        ...
        <div className="max-h-80 overflow-y-auto px-5 py-3">
          <div className="mb-2 flex items-center justify-between text-[10px] uppercase tracking-wider text-up-dim">
            <span>candidatos · <span className="text-up-orange tabular-nums">{picked.length}</span> selecionados {picked.length < 2 && '(mínimo 2)'}</span>
            {q && <span>{filtered.length} resultado{filtered.length === 1 ? '' : 's'}</span>}
          </div>
          {filtered.length === 0 ? (
            <div className="py-6 text-center text-[11px] text-up-dim">nenhum jogo pra "{query}"</div>
          ) : (
